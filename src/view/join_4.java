	...
	public void SET_Label_Area() {// 기타 라벨 영역
		...
		jl[1].addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				char[] pass1 = ((JPasswordField) jta[3]).getPassword();
				...
				if (userId instanceof String && userPwd instanceof String && userName instanceof String
						&& userPhone instanceof String) {
					if (!userPwd.equals(userPwd1)) {
						JOptionPane.showMessageDialog(null, "비밀번호를 같게 입력하세요", "삐~", JOptionPane.WARNING_MESSAGE);

					} else if (user.insertUser(userId, userPwd, userName, userPhone, "0") == true) {
					...
					} else if (userId.equals("") || userPwd.equals("") || userName.equals("") || userPhone.equals("")) {
						JOptionPane.showMessageDialog(null, "빈 칸이 있습니다.", "삐~", JOptionPane.WARNING_MESSAGE);
					} else if (user.insertUser(userId, userPwd, userName, userPhone, "0") == false) {
						JOptionPane.showMessageDialog(null, "이미 있는 아이디입니다.", "삐~", JOptionPane.WARNING_MESSAGE);
					}

				} else {
					JOptionPane.showMessageDialog(null, "잘못 입력하셨습니다.", "삐~", JOptionPane.WARNING_MESSAGE);
				}
